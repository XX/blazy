...
### 20.6 LOD: теперь у него появилась работа
...
Теперь картина другая. Сценарий «пан на Box-LOD» (зум 0.15, в кадре 741 нода вместо
27) стоит 3.75 мс против 0.672 мс при полной детализации. Но это **не регресс**: на
дальнем зуме видно в 22 раза больше графа, а стоимость выросла в 5.6 раза. В пересчёте
на ноду — 5 мкс против 20 мкс, то есть LOD экономит ровно то, ради чего вводился.

Что осталось недоделанным и стоит следующим шагом: при `Detail::Box` `GraphNode`
**сташит** слайдер и чекбокс, а не удаляет их. По §20.2 сташенный виджет всё равно
обходится проходами, так что 741 нода — это 2223 виджета в дереве. Вторая ступень
виртуализации (не создавать контролы, пока детализация ниже `Full`) должна убрать
из этого сценария ещё примерно две трети.
...
* **LOD — после второй ступени виртуализации** (§20.6), тогда же и перемерить.
