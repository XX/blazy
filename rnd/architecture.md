...
> по **всему дереву виджетов**, а не по видимой части. Culling обязателен, но
> **недостаточен**. Полные числа и разбор — в §20.
...
> **ИСПРАВЛЕНО по результатам Фазы 0.** Здесь было написано: «невидимые дети
> помечаются `is_stashed` — Masonry их не красит и не добавляет в сцену слоя».
> Это верно, но недостаточно: `set_stashed` **не убирает ноду из рекурсии проходов**,
> и стоимость кадра остаётся линейной по общему числу нод (§20.2). Нужна
> **виртуализация**: нода вне вида должна удаляться из дерева, а её состояние —
> жить в модели графа. Образец — `masonry::widgets::virtual_scroll`. Тезис
> «10k нод при 40 видимых должны стоить как 40» остаётся целью, но достигается
> не culling'ом.
...
### 20.3 Следствие: нужна виртуализация, а не culling
...
Это ровно то, что делает `masonry::widgets::virtual_scroll`: создаёт и удаляет детей
по мере прокрутки, а не прячет их. Для нод-канваса это означает:

* модель графа — источник истины (позиции, размеры, значения контролов);
* виджеты создаются только для нод в видимом прямоугольнике плюс запас;
* при выходе из вида виджет удаляется, состояние остаётся в модели;
* пул виджетов для переиспользования, чтобы не аллоцировать на каждом кадре пана.

Оценка потолка есть в самой таблице: виртуализированный канвас на 5000 нод должен
стоить как нынешний на ~250 (0.22 мс idle, 0.81 мс пан), то есть примерно
**в 26 раз дешевле**. Это делает 5000 нод не просто рабочим сценарием, а сценарием
с большим запасом.

Цена — сложность: нужен слой модели и рециклинг виджетов. Но это работа Фазы 4
(§16, `blazy-canvas` в продовом виде), а не отказ от Masonry.

### 20.4 LOD не окупился — и это ожидаемо

`21.05` мс при полной детализации против `20.90` при Box-LOD. Разница в пределах шума.

Это прямое следствие §20.2: LOD экономит команды отрисовки и layout содержимого, но
не обход дерева, который здесь доминирует. **LOD имеет смысл вводить только после
виртуализации** — тогда он начнёт экономить на том, что действительно останется в
стоимости кадра. Сейчас его сложность себя не оправдывает.

Реализация в примере оставлена: она корректна, идиоматична (через систему свойств
Masonry — `CanvasDetail` + `property_changed`), и после виртуализации её нужно будет
перемерить, а не переписывать.

### 20.5 Вердикт по Фазе 0

**Masonry берём.** Ни один из провалов не связан с абстракцией рендера, retained-
деревом, трансформами, системой свойств или маршрутизацией событий — то есть со всем
тем, ради чего Masonry и выбирался. Проблема ровно одна: обход дерева, и она решается
на нашей стороне, без форка и без патчей в апстрим.

Что меняется в плане §16:

* **`blazy-canvas` строится сразу виртуализированным.** «Culling через `set_stashed`»
  из §10.2 — тупик; переписать раздел под модель + рециклинг виджетов.
* **LOD — после виртуализации,** и только если замеры покажут смысл.
* **Уровень B (§7.3) становится ещё важнее.** Обход дерева — это стоимость *на окно*.
  Шесть областей с крупными деревьями сложатся аддитивно, и per-area текстуры не
  спасут: обход происходит до всякой отрисовки. Значит виртуализация нужна не только
  канвасу, но и любому редактору с длинными списками (аутлайнер, таймлайн).
* **Добавить в CI бенчмарк из `examples/node-canvas`.** Развёртка по числу нод —
  это регрессионный тест на всю архитектуру, а не микробенчмарк.
...
проблему, а не фундаментальное ограничение — достаточно сбрасывать флаги при
unstash, что `update_stashed` уже и делает (`state.set_needs_layout(true)` и
компания). Это [xilem#789](https://github.com/linebender/xilem/issues/789) по духу,
хотя и не по букве.
