...
### 20.7 LOD: работа нашлась, но не та
...
После дальнего поля роль `Detail::Box` изменилась: это больше не «рисовать проще», а
**порог, ниже которого виджетов нет вообще**. Промежуточный уровень `Simplified`
(сташить чекбокс, оставить слайдер) остаётся полумерой ровно по §20.2 — сташенный
виджет всё равно обходится проходами. Его стоит либо переделать на удаление детей,
либо убрать: сейчас он не окупается, и замеры это показывают.

### 20.7 Вердикт по Фазе 0
...
* **Промежуточный LOD пересмотреть или убрать** (§20.7).
