...
### 20.7a Full: контролы только у ноды под курсором

Уровень `Simplified` снял стоимость там, где ноды мелкие. В `Full` контролы нужны —
но нужны они одной ноде за раз. Пользователь в любой момент работает с одной, а платит
за все: при 140 нодах в кадре это 560 виджетов, из которых 557 никто не трогает.

Уровень детализации стал **свойством ноды, а не канваса**:
...
`CanvasLayer` отслеживает ноду под курсором и толкает её вниз как `active`; `Slot`
помнит, на каком уровне собран его виджет, так что устаревшие пересобираются точечно.
Проверка устаревания идёт только когда детализация или активная нода менялись — во
время обычного панорамирования она пропускается целиком.

| Панорамирование (5000 нод) | до §20.7a | после |
|---|---:|---:|
| зум 1.0 (34 ноды, Full) | 0.455 мс | **0.136 мс** |
| зум 0.40 (140 нод, Full) | 1.656 мс | **0.348 мс** |
| зум 0.15 (741 нода) | 1.707 мс | **1.461 мс** |
| зум 0.11 (792 ноды) | 2.054 мс | **1.784 мс** |
| idle | 0.068 мс | **0.041 мс** |

**Цена приёма — визуальный скачок при наведении.** Нарисованная заглушка должна
совпадать с настоящими контролами по месту и виду, иначе интерфейс дёргается под
курсором. Отсюда вторая находка: заглушка сама по себе не бесплатна. Сцена каждой ноды
вклеивается в сцену слоя каждый кадр (§7.2), поэтому лишняя команда отрисовки
умножается на число видимых нод. Скруглённые прямоугольники и окружность в заглушке
стоили +0.4 мс при 741 ноде — больше, чем экономил весь приём на этом масштабе.

Решение: заглушка выбирает детальность по **глобальному** уровню, который приходит в
свойстве `CanvasDetail`. При `Full` нод мало и заглушка рисует ползунок с движком и
чекбокс; при `Simplified` их сотни, и остаются две плашки. Это разделение стоит
запомнить: `CanvasDetail` — «что показывает канвас в целом», аргумент `build` — «чем
является эта нода».

**Следствие для UX, которое надо принять сознательно:** контрол нельзя нажать, не
наведя на ноду. Мышью это происходит само собой, но клавиатурная навигация и тач
потребуют отдельного решения — например, материализации контролов у выделенных нод.
Тест `controls_write_back_to_the_model` теперь проходит именно этот путь: наведение,
затем клик.
