...
//! the same graph. There is no counter overlay here, unlike the node-canvas window:
//! the questions this experiment asks are about how many areas re-lay-out, which is
//! a number you read off the benchmark rather than off the screen. See
//! [`area_screen`] for what is under test.
...
fn main() {
    let args = Args::parse();
    let (screen, _graph) = build_screen(args.areas.max(1), args.nodes);
